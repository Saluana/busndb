...
describe('Relationship Testing', () => {
  ...
  let postCategories: ReturnType<typeof db.collection<typeof postCategorySchema>>;
  ...
  describe('One-to-Many Relationships', () => {
    test('should handle user-posts relationship', () => {
      ...
      const alicePosts = posts.where('authorId').eq(user1.id).toArraySync();
    ...
    test('should handle post-comments relationship', () => {
      ...
      const postComments = comments.where('postId').eq(post.id).toArraySync();
      expect(postComments).toHaveLength(2);
      expect(postComments.every(c => c.postId === post.id)).toBe(true);

      const anotherPostComments = comments.where('postId').eq(anotherPost.id).toArraySync();
      expect(anotherPostComments).toHaveLength(1);
      expect(anotherPostComments[0].content).toBe('Comment on another post');
    });
    ...
    test('should handle cascading deletes simulation', () => {
      ...
      const userComments = comments.where('authorId').eq(user.id).toArraySync();
  ...
  describe('One-to-One Relationships', () => {
    test('should handle user-profile relationship', () => {
      ...
      const userProfile = profiles.where('userId').eq(user.id).firstSync();
      ...
      const existingProfile = profiles.where('userId').eq(user.id).firstSync();
      expect(existingProfile).not.toBeNull();

      // Attempting to create another profile should be prevented by application logic
      const duplicateProfileCheck = profiles.where('userId').eq(user.id).countSync();
    ...
    test('should handle profile updates and deletion', () => {
      ...
      const deletedProfile = profiles.where('userId').eq(user.id).firstSync();
  ...
  describe('Many-to-Many Relationships', () => {
    test('should handle post-category many-to-many relationship', () => {
      ...
      const techPostIds = postCategories
        .where('categoryId').eq(techCategory.id)
        .toArraySync()
        .map(pc => pc.postId);
      ...
      const programmingPosts = posts
        .where('id')
        .inSubquery(
          postCategories.where('categoryId').eq(programmingCategory.id).select('postId'),
          'post_categories'
        )
        .toArraySync();
    ...
    test('should handle post-tag many-to-many relationship', () => {
      ...
      const remainingTags = postTags.where('postId').eq(post.id).countSync();
    ...
    test('should handle complex many-to-many queries', () => {
      ...
      const backendPosts = postTags
        .where('tagId').eq(backendTag.id)
        .toArraySync()
        .map(pt => pt.postId);

      const bothTagsPosts = frontendPosts.filter(id => backendPosts.includes(id));
  ...
  describe('Edge Cases and Complex Scenarios', () => {
    ...
    test('should handle circular references in relationships', () => {
      ...
      const user1Comments = comments.where('authorId').eq(user1.id).toArraySync();
      const user2Comments = comments.where('authorId').eq(user2.id).toArraySync();
    ...
    test('should handle null foreign keys', () => {
      ...
      const orphanPosts = allPosts.filter(p => !validUserIds.includes(p.authorId));
    ...
    test('should handle complex filtering across relationships', () => {
      ...
      // Categorize posts
      postCategories.insertSync({ postId: post1.id, categoryId: techCategory.id });
      postCategories.insertSync({ postId: post2.id, categoryId: techCategory.id });
      postCategories.insertSync({ postId: post3.id, categoryId: scienceCategory.id });

      // Find published posts by authors over 25 in Tech category
      const eligibleAuthors = users.where('age').gt(25).toArraySync().map(u => u.id);
      const publishedPosts = posts
        .where('published').eq(true)
        .and()
        .where('authorId').in(eligibleAuthors)
        .toArraySync();
      ...
      const result = publishedPosts.filter(p => techPostIds.includes(p.id));
  ...
  describe('JOIN Operations', () => {
    test('should perform INNER JOIN between users and posts', () => {
      ...
      const joinResults = users
        .where('id').exists() // Start with a simple condition to get QueryBuilder
      ...
      const aliceResult = joinResults.find(r => r.name === 'Alice Johnson');
    ...
    test('should perform complex JOIN with filtering', () => {
      ...
      const filteredJoinResults = users
        .where('age').gte(25) // Start with filtering condition
        .join('posts', 'id', 'authorId')
        .where('posts.published').eq(true) // Use table-prefixed field name
        .select('users.name', 'users.age', 'posts.title', 'posts.published')
        .toArraySync();
    ...
    test('should perform multiple JOINs for complex relationships', () => {
      ...
      const multiJoinResults = users
        .where('name').eq('Multi Join User') // Start with filtering condition
  ...
  describe('CASCADE Operations', () => {
    ...
    beforeEach(() => {
      ...
      usersWithCascade = db.collection('users_cascade', userSchemaWithConstraints);
      
      postsWithCascade = db.collection('posts_cascade', postSchemaWithConstraints, {
        constrainedFields: {
          'authorId': {
            type: 'TEXT',
    ...
    test('should handle CASCADE DELETE foreign key constraints', () => {
      ...
      // Verify posts exist
      expect(postsWithCascade.where('authorId').eq(user.id).countSync()).toBe(2);
      ...
      expect(postsWithCascade.where('authorId').eq(user.id).countSync()).toBe(0);
    ...
    test('should handle SET NULL on delete when configured', () => {
      ...
      const commentsWithSetNull = db.collection('comments_set_null', commentsSchemaWithSetNull, {
        constrainedFields: {
          'authorId': {
            type: 'TEXT',
            foreignKey: 'users_cascade.id',
    ...
    test('should handle RESTRICT constraint (prevent deletion)', () => {
      ...
      const postsWithRestrict = db.collection('posts_restrict', restrictPostsSchema, {
        constrainedFields: {
          'authorId': {
            type: 'TEXT',
            foreignKey: 'users_cascade.id',
  ...
  describe('Performance with Relationships', () => {
    test('should handle large relationship datasets efficiently', () => {
      ...
      for (let i = 0; i < userCount; i++) {
        userPromises.push(users.insertSync({
          name: `User ${i}`,
      ...
      allUsers.forEach((user, index) => {
        ...
        for (let j = 0; j < postCount; j++) {
          postPromises.push(posts.insertSync({
      ...
      const publishedPosts = posts.where('published').eq(true).toArraySync();
    ...
    test('should handle relationship queries with proper indexing simulation', () => {
      ...
      const authorPosts = posts.where('authorId').eq(author.id).toArraySync();
