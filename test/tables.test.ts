...
describe('Tables: BusNDB API vs Raw SQL Verification', () => {
    ...
    describe('Async Operations', () => {
        ...
        test('indexes and constraints verification', async () => {
            ...
            const indexes = await db.query("SELECT name, tbl_name FROM sqlite_master WHERE type='index' AND name NOT LIKE 'sqlite_%'");
            ...
            const postsForeignKeys = await db.query("PRAGMA foreign_key_list(posts)");
            ...
            const categoriesForeignKeys = await db.query("PRAGMA foreign_key_list(categories)");
        ...
        test('insert operations: API vs Raw SQL verification', async () => {
            ...
            const rawUserRows = await db.query("SELECT * FROM users WHERE _id = ?", [user.id]);
            ...
            const rawPostRows = await db.query("SELECT * FROM posts WHERE _id = ?", [post.id]);
        ...
        test('bulk insert operations verification', async () => {
            ...
            const allUsersRaw = await db.query("SELECT * FROM users ORDER BY name");
        ...
        test('query operations: API vs Raw SQL verification', async () => {
            ...
            const developersAPI = await users.where('name').like('Developer%').toArray();
            const developersSQL = await db.query("SELECT * FROM users WHERE name LIKE 'Developer%'");
            ...
            const youngUsersSQL = await db.query("SELECT * FROM users WHERE age < 35");
            ...
            const orderedSQL = await db.query("SELECT * FROM users ORDER BY age DESC");
        ...
        test('delete operations: API vs Raw SQL verification', async () => {
            // Insert test users
            const users1 = await users.insert({ name: 'User 1', email: 'user1@example.com' });
            const users2 = await users.insert({ name: 'User 2', email: 'user2@example.com' });
            const users3 = await users.insert({ name: 'User 3', email: 'user3@example.com' });
            
            // Verify initial count
            const initialCount = await db.query("SELECT COUNT(*) as count FROM users");
            ...
            // Verify using raw SQL
            const remainingUsers = await db.query("SELECT * FROM users ORDER BY name");
            ...
            const deletedUserRows = await db.query("SELECT * FROM users WHERE _id = ?", [users2.id]);
            expect(deletedUserRows.length).toBe(0);
        });

        test('complex relationship queries', async () => {
            // Create users
            const author1 = await users.insert({ name: 'Author 1', email: 'author1@example.com' });
            const author2 = await users.insert({ name: 'Author 2', email: 'author2@example.com' });
            ...
            const author1Posts = await posts.where('authorId').eq(author1.id).toArray();
            expect(author1Posts.length).toBe(2);
            
            // Query posts by author using raw SQL
            const author1PostsSQL = await db.query("SELECT * FROM posts WHERE authorId = ?", [author1.id]);
            expect(author1PostsSQL.length).toBe(2);
            
            // Query high-view posts using API
            const popularPosts = await posts.where('viewCount').gte(10).toArray();
            expect(popularPosts.length).toBe(2);
            
            // Verify with raw SQL
            const popularPostsSQL = await db.query("SELECT * FROM posts WHERE viewCount >= 10");
    ...
    describe('Sync Operations', () => {
        test('table creation and structure verification (sync)', () => {
            // Check if tables were created
            const tables = db.querySync("SELECT name FROM sqlite_master WHERE type='table' ORDER BY name");
            const tableNames = tables.map(row => row.name);
        ...
        test('insert operations: sync API vs Raw SQL verification', () => {
            ...
            // Verify using raw SQL
            const rawUserRows = db.querySync("SELECT * FROM users WHERE _id = ?", [user.id]);
            expect(rawUserRows.length).toBe(1);
        ...
        test('bulk insert operations verification (sync)', () => {
            ...
            const userData = [
                { name: 'Alice Sync', email: 'alice.sync@example.com', age: 25 },
                { name: 'Bob Sync', email: 'bob.sync@example.com', age: 35 },
                { name: 'Carol Sync', email: 'carol.sync@example.com', age: 28 },
            ];
            ...
            // Verify count using raw SQL
            const countResult = db.querySync("SELECT COUNT(*) as count FROM users");
            expect(countResult[0].count).toBe(3);
            
            // Verify all users exist using raw SQL
            const allUsersRaw = db.querySync("SELECT * FROM users ORDER BY name");
            expect(allUsersRaw.length).toBe(3);
        ...
        test('query operations: sync API vs Raw SQL verification', () => {
            // Setup test data
            const testUsers = [
                { name: 'Developer Dan Sync', email: 'dan.sync@dev.com', age: 30 },
                { name: 'Designer Dana Sync', email: 'dana.sync@design.com', age: 28 },
                { name: 'Manager Mike Sync', email: 'mike.sync@mgmt.com', age: 45 },
            ];
            
            const insertedUsers = users.insertBulkSync(testUsers);
            
            // Test simple where query
            const developersAPI = users.where('name').like('Developer%').toArraySync();
            const developersSQL = db.querySync("SELECT * FROM users WHERE name LIKE 'Developer%'");
            ...
            const youngUsersSQL = db.querySync("SELECT * FROM users WHERE age < 35");
            ...
            const orderedSQL = db.querySync("SELECT * FROM users ORDER BY age DESC");
        ...
        test('delete operations: sync API vs Raw SQL verification', () => {
            // Insert test users
            const users1 = users.insertSync({ name: 'User 1 Sync', email: 'user1.sync@example.com' });
            const users2 = users.insertSync({ name: 'User 2 Sync', email: 'user2.sync@example.com' });
            const users3 = users.insertSync({ name: 'User 3 Sync', email: 'user3.sync@example.com' });
            
            // Verify initial count
            const initialCount = db.querySync("SELECT COUNT(*) as count FROM users");
            expect(initialCount[0].count).toBe(3);
            ...
            // Verify using raw SQL
            const remainingUsers = db.querySync("SELECT * FROM users ORDER BY name");
            expect(remainingUsers.length).toBe(2);
        ...
        test('complex relationship queries (sync)', () => {
            // Create users
            const author1 = users.insertSync({ name: 'Author 1 Sync', email: 'author1.sync@example.com' });
            const author2 = users.insertSync({ name: 'Author 2 Sync', email: 'author2.sync@example.com' });
            ...
            const author1Posts = posts.where('authorId').eq(author1.id).toArraySync();
            expect(author1Posts.length).toBe(2);
            
            // Query posts by author using raw SQL
            const author1PostsSQL = db.querySync("SELECT * FROM posts WHERE authorId = ?", [author1.id]);
            expect(author1PostsSQL.length).toBe(2);
            ...
            // Verify with raw SQL
            const popularPostsSQL = db.querySync("SELECT * FROM posts WHERE viewCount >= 10");
