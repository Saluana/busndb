...
function benchmarkSync(name: string, count: number, fn: () => void): BenchmarkResult {
...
async function benchmarkAsync(name: string, count: number, fn: () => Promise<void>): Promise<BenchmarkResult> {
    const start = performance.now();
    await fn();
    const end = performance.now();
    const totalTime = end - start;
...
function compareResults(syncResult: BenchmarkResult, asyncResult: BenchmarkResult) {
    ...
    console.log(`  ${faster} is ${percentage.toFixed(1)}% ${speedup > 1 ? 'faster' : 'slower'}`);
    console.log(`  Sync: ${syncResult.opsPerSecond.toLocaleString()} ops/sec`);
    console.log(`  Async: ${asyncResult.opsPerSecond.toLocaleString()} ops/sec`);
}
...
test('Upsert Benchmark - Comprehensive Sync vs Async', async () => {
    ...
    const newIds = Array.from({ length: benchmarkCount }, () => crypto.randomUUID());
    const updateIds = existingIds.slice(0, benchmarkCount);
    const testIds = [...updateIds, ...newIds];
    
    console.log(`\nBenchmarking ${benchmarkCount * 2} operations (${benchmarkCount} updates + ${benchmarkCount} inserts)...`);
    
    // === Single Upsert Operations ===
    
    // Sync single upserts
    const syncSingleResult = benchmarkSync('Single Upserts', testIds.length, () => {
    ...
    const asyncSingleResult = await benchmarkAsync('Single Upserts', testIds.length, async () => {
    ...
    const asyncConcurrentResult = await benchmarkAsync('Single Upserts (Concurrent)', testIds.length, async () => {
        const promises = testIds.map((id, i) => 
            collection.upsert(id, {
                ...
                metadata: i % 2 === 0 ? {
                    level: i % 5,
                    tags: [`concurrent${i % 3}`, `parallel${i % 4}`],
                } : undefined,
            })
        );
    ...
    console.log(`  Sequential: ${asyncSingleResult.opsPerSecond.toLocaleString()} ops/sec`);
    console.log(`  Concurrent: ${asyncConcurrentResult.opsPerSecond.toLocaleString()} ops/sec`);
    const concurrentSpeedup = asyncConcurrentResult.opsPerSecond / asyncSingleResult.opsPerSecond;
    console.log(`  Concurrent is ${concurrentSpeedup.toFixed(1)}x ${concurrentSpeedup > 1 ? 'faster' : 'slower'} than sequential`);
    console.log(`  Concurrent vs Sync: ${(asyncConcurrentResult.opsPerSecond / syncSingleResult.opsPerSecond).toFixed(1)}x speedup`);
    ...
    // Async bulk upserts
    const asyncBulkResult = await benchmarkAsync('Bulk Upserts', 1, async () => {
    ...
    const mixedTestIds = [
        ...
        ...Array.from({ length: benchmarkCount / 2 }, () => crypto.randomUUID()) // Half inserts
    ];
    
    // Sync mixed operations
    const syncMixedResult = benchmarkSync('Mixed Insert/Update', mixedTestIds.length, () => {
    ...
    const asyncMixedResult = await benchmarkAsync('Mixed Insert/Update', mixedTestIds.length, async () => {
    ...
    const asyncConcurrentMixedResult = await benchmarkAsync('Mixed Insert/Update (Concurrent)', mixedTestIds.length, async () => {
    ...
    console.log(`  Sequential: ${asyncMixedResult.opsPerSecond.toLocaleString()} ops/sec`);
    console.log(`  Concurrent: ${asyncConcurrentMixedResult.opsPerSecond.toLocaleString()} ops/sec`);
    const mixedConcurrentSpeedup = asyncConcurrentMixedResult.opsPerSecond / asyncMixedResult.opsPerSecond;
    console.log(`  Concurrent is ${mixedConcurrentSpeedup.toFixed(1)}x ${mixedConcurrentSpeedup > 1 ? 'faster' : 'slower'} than sequential`);
    console.log(`  Concurrent vs Sync: ${(asyncConcurrentMixedResult.opsPerSecond / syncMixedResult.opsPerSecond).toFixed(1)}x speedup`);
    ...
    const asyncUpsertResult = await collection.upsert(testId, { ...testDoc, score: 200 });
...
test('Upsert Correctness - Sync vs Async Equivalence', async () => {
    ...
    console.log(`Sync insert result: ID=${syncInsertResult.id}, score=${syncInsertResult.score}`);
    ...
    console.log(`Async update result: ID=${asyncUpdateResult.id}, score=${asyncUpdateResult.score}`);
