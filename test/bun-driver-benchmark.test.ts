...
describe('Bun Driver Performance Benchmark', () => {
    ...
    beforeEach(async () => {
        ...
        await bunDriver.exec(
            'CREATE INDEX IF NOT EXISTS idx_age ON bun_benchmark_test(age)'
        );
        await bunDriver.exec(
            'CREATE INDEX IF NOT EXISTS idx_score ON bun_benchmark_test(score)'
        );
        await bunDriver.exec(
            'CREATE INDEX IF NOT EXISTS idx_active ON bun_benchmark_test(active)'
        );
    });
    ...
    async function benchmarkAsyncInserts() {
        ...
        console.log(
            `Async Inserts: ${TEST_SIZE} operations in ${insertTime.toFixed(
                2
            )}ms`
        );
        console.log(`Avg per insert: ${(insertTime / TEST_SIZE).toFixed(4)}ms`);
        console.log(
            `Inserts/sec: ${Math.round(TEST_SIZE / (insertTime / 1000))}`
        );
    ...
    async function benchmarkSyncInserts() {
        ...
        console.log(
            `Sync Inserts: ${TEST_SIZE} operations in ${insertTime.toFixed(
                2
            )}ms`
        );
        console.log(`Avg per insert: ${(insertTime / TEST_SIZE).toFixed(4)}ms`);
    ...
    async function benchmarkAsyncReads() {
        ...
        for (let i = 0; i < 2000; i++) {
            ...
            await bunDriver.query(
                'SELECT * FROM bun_benchmark_test WHERE id = ?',
                [randomId]
            );
        }
        ...
        for (let i = 0; i < 200; i++) {
            ...
            await bunDriver.query(
                'SELECT * FROM bun_benchmark_test WHERE age BETWEEN ? AND ?',
                [minAge, minAge + 10]
            );
        }
        ...
        await bunDriver.query(
            'SELECT COUNT(*) as total, AVG(score) as avg_score FROM bun_benchmark_test'
        );
        const fullScanTime = performance.now() - fullScanStart;

        console.log(
            `Point Queries: 2000 operations in ${pointQueryTime.toFixed(2)}ms`
        );
        console.log(
            `Queries/sec: ${Math.round(2000 / (pointQueryTime / 1000))}`
        );
        console.log(
            `Range Queries: 200 operations in ${rangeQueryTime.toFixed(2)}ms`
        );
        console.log(`Range/sec: ${Math.round(200 / (rangeQueryTime / 1000))}`);
        console.log(
            `Complex Queries: 100 operations in ${complexQueryTime.toFixed(
                2
            )}ms`
        );
        console.log(
            `Complex/sec: ${Math.round(100 / (complexQueryTime / 1000))}`
        );
        ...
        return {
            pointQueries: {
                time: pointQueryTime,
                opsPerSec: 2000 / (pointQueryTime / 1000),
            },
            rangeQueries: {
                time: rangeQueryTime,
                opsPerSec: 200 / (rangeQueryTime / 1000),
            },
            complexQueries: {
                time: complexQueryTime,
                opsPerSec: 100 / (complexQueryTime / 1000),
            },
    ...
    async function benchmarkSyncReads() {
        ...
        console.log(
            `Sync Point Queries: 2000 operations in ${pointQueryTime.toFixed(
                2
            )}ms`
        );
        console.log(
        ...
        console.log(
            `Sync Range Queries: 200 operations in ${rangeQueryTime.toFixed(
                2
            )}ms`
        );
        console.log(`Range/sec: ${Math.round(200 / (rangeQueryTime / 1000))}`);
    ...
    async function benchmarkUpdates() {
        ...
        console.log(
            `Async Updates: 1000 operations in ${asyncUpdateTime.toFixed(2)}ms`
        );
        console.log(
            `Async Updates/sec: ${Math.round(1000 / (asyncUpdateTime / 1000))}`
        );
        console.log(
            `Sync Updates: 1000 operations in ${syncUpdateTime.toFixed(2)}ms`
        );
        console.log(
            `Sync Updates/sec: ${Math.round(1000 / (syncUpdateTime / 1000))}`
        );

        return {
            async: {
                time: asyncUpdateTime,
                opsPerSec: 1000 / (asyncUpdateTime / 1000),
            },
            sync: {
                time: syncUpdateTime,
                opsPerSec: 1000 / (syncUpdateTime / 1000),
            },
        };
    }
    ...
    async function benchmarkDeletes() {
        ...
        for (let i = 0; i < deleteCount / 2; i++) {
            const randomId = Math.floor(Math.random() * TEST_SIZE) + 1;
            await bunDriver.exec(
                'DELETE FROM bun_benchmark_test WHERE id = ?',
                [randomId]
            );
        }
        ...
        for (let i = 0; i < deleteCount / 2; i++) {
            const randomId = Math.floor(Math.random() * TEST_SIZE) + 1;
            bunDriver.execSync('DELETE FROM bun_benchmark_test WHERE id = ?', [
                randomId,
            ]);
        }
        ...
        console.log(
            `Async Deletes: ${
                deleteCount / 2
            } operations in ${asyncDeleteTime.toFixed(2)}ms`
        );
        console.log(
            `Async Deletes/sec: ${Math.round(
                deleteCount / 2 / (asyncDeleteTime / 1000)
            )}`
        );
        console.log(
            `Sync Deletes: ${
                deleteCount / 2
            } operations in ${syncDeleteTime.toFixed(2)}ms`
        );
        console.log(
            `Sync Deletes/sec: ${Math.round(
                deleteCount / 2 / (syncDeleteTime / 1000)
            )}`
        );

        return {
            async: {
                time: asyncDeleteTime,
                opsPerSec: deleteCount / 2 / (asyncDeleteTime / 1000),
            },
            sync: {
                time: syncDeleteTime,
                opsPerSec: deleteCount / 2 / (syncDeleteTime / 1000),
            },
        };
    }
    ...
    async function benchmarkTransactions() {
        ...
        await bunDriver.transaction(async () => {
            for (let i = 0; i < 500; i++) {
                ...
                await bunDriver.exec(
                    'UPDATE bun_benchmark_test SET score = ? WHERE name = ?',
                    [75.0, `TxUser${i}`]
                );
            }
        });
        ...
        console.log(
            `Ops/sec in transaction: ${Math.round(
                1000 / (transactionTime / 1000)
            )}`
        );
    ...
    async function benchmarkConcurrentOperations() {
        ...
        const operations = [
            ...
            async () => {
                for (let i = 0; i < 100; i++) {
                    await bunDriver.query(
                        'SELECT * FROM bun_benchmark_test WHERE active = ? LIMIT 10',
                        [true]
                    );
                }
            },
            async () => {
                for (let i = 0; i < 50; i++) {
                    ...
                    await bunDriver.exec(
                        'UPDATE bun_benchmark_test SET score = ? WHERE id = ?',
                        [Math.random() * 100, randomId]
                    );
                }
            },
        ];
        ...
        console.log(
            `Concurrent Operations (250 total): ${concurrentTime.toFixed(2)}ms`
        );
        console.log(
            `Concurrent Ops/sec: ${Math.round(250 / (concurrentTime / 1000))}`
        );
    ...
    test('should benchmark Bun driver performance comprehensively', async () => {
        ...
        console.log(`\nInsert Performance:`);
        console.log(
            `  Async: ${results.asyncInserts.opsPerSec.toFixed(0)} ops/sec`
        );
        console.log(
            `  Sync:  ${results.syncInserts.opsPerSec.toFixed(0)} ops/sec`
        );
        const insertRatio =
            results.syncInserts.opsPerSec / results.asyncInserts.opsPerSec;
        console.log(
            `  Sync is ${
                insertRatio > 1
                    ? insertRatio.toFixed(2) + 'x faster'
                    : (1 / insertRatio).toFixed(2) + 'x slower'
            } than async`
        );

        console.log(`\nRead Performance:`);
        console.log(
            `  Async Point Queries: ${results.asyncReads.pointQueries.opsPerSec.toFixed(
                0
            )} ops/sec`
        );
        console.log(
            `  Sync Point Queries:  ${results.syncReads.pointQueries.opsPerSec.toFixed(
                0
            )} ops/sec`
        );
        const readRatio =
            results.syncReads.pointQueries.opsPerSec /
            results.asyncReads.pointQueries.opsPerSec;
        console.log(
            `  Sync is ${
                readRatio > 1
                    ? readRatio.toFixed(2) + 'x faster'
                    : (1 / readRatio).toFixed(2) + 'x slower'
            } than async`
        );

        console.log(`\nUpdate Performance:`);
        console.log(
            `  Async: ${results.updates.async.opsPerSec.toFixed(0)} ops/sec`
        );
        console.log(
            `  Sync:  ${results.updates.sync.opsPerSec.toFixed(0)} ops/sec`
        );

        console.log(`\nDelete Performance:`);
        console.log(
            `  Async: ${results.deletes.async.opsPerSec.toFixed(0)} ops/sec`
        );
        console.log(
            `  Sync:  ${results.deletes.sync.opsPerSec.toFixed(0)} ops/sec`
        );

        console.log(`\nAdvanced Operations:`);
        console.log(
            `  Transactions: ${results.transactions.opsPerSec.toFixed(
                0
            )} ops/sec`
        );
        console.log(
            `  Concurrent:   ${results.concurrent.opsPerSec.toFixed(0)} ops/sec`
        );

        console.log(`\nOverall Assessment:`);
        const avgAsyncPerf =
            (results.asyncInserts.opsPerSec +
                results.asyncReads.pointQueries.opsPerSec +
                results.updates.async.opsPerSec) /
            3;
        const avgSyncPerf =
            (results.syncInserts.opsPerSec +
                results.syncReads.pointQueries.opsPerSec +
                results.updates.sync.opsPerSec) /
            3;
        console.log(
            `  Average Async Performance: ${avgAsyncPerf.toFixed(0)} ops/sec`
        );
        console.log(
            `  Average Sync Performance:  ${avgSyncPerf.toFixed(0)} ops/sec`
        );

        if (avgSyncPerf > avgAsyncPerf) {
            console.log(
                `  🏆 Sync operations are ${(
                    avgSyncPerf / avgAsyncPerf
                ).toFixed(2)}x faster on average`
            );
        } else {
            console.log(
                `  🏆 Async operations are ${(
                    avgAsyncPerf / avgSyncPerf
                ).toFixed(2)}x faster on average`
            );
        }
