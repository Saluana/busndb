import { z } from 'zod';
import type { DBConfig, Driver, InferSchema } from './types.js';
import { BunDriver } from './drivers/bun.js';
...
export class Database {
  ...
  private createDriver(config: DBConfig): Driver {
    const driver = config.driver || (typeof Bun !== 'undefined' ? 'bun' : 'node');
  ...
  collection<T extends z.ZodSchema>(
    ...
    if (schema) {
      ...
      const collectionSchema = this.registry.register(name, schema, options);
